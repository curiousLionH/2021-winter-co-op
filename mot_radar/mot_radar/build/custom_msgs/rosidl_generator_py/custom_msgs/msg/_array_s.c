...
bool custom_msgs__msg__array__convert_from_py(PyObject * _pymsg, void * _ros_message)
{
  // check that the passed message is of the expected Python class
  {
    ...
    assert(strncmp("custom_msgs.msg._array.Array", full_classname_dest, 28) == 0);
  }
  custom_msgs__msg__Array * ros_message = _ros_message;
  {  // x
  ...
  {  // h
    ...
    if (!rosidl_runtime_c__float__Sequence__init(&(ros_message->h), size)) {
      PyErr_SetString(PyExc_RuntimeError, "unable to create float__Sequence ros_message");
      Py_DECREF(seq_field);
      Py_DECREF(field);
      return false;
    }
    float * dest = ros_message->h.data;
    for (Py_ssize_t i = 0; i < size; ++i) {
      PyObject * item = PySequence_Fast_GET_ITEM(seq_field, i);
      if (!item) {
        Py_DECREF(seq_field);
...
PyObject * custom_msgs__msg__array__convert_to_py(void * raw_ros_message)
{
  ...
  {
    PyObject * pymessage_module = PyImport_ImportModule("custom_msgs.msg._array");
  ...
  {  // h
    ...
    if (ros_message->h.size > 0) {
      ...
      PyObject * frombytes = PyObject_GetAttrString(field, "frombytes");
      assert(frombytes != NULL);
      float * src = &(ros_message->h.data[0]);
      PyObject * data = PyBytes_FromStringAndSize((const char *)src, ros_message->h.size * sizeof(float));
      assert(data != NULL);
      PyObject * ret = PyObject_CallFunctionObjArgs(frombytes, data, NULL);
      Py_DECREF(data);
      Py_DECREF(frombytes);
