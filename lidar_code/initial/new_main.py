...
def make_object_list(cluster_data):
    ...
    for i in range(max_label+1):
        ...
        if len(object_data) != 0:
            ...
            box_x = (object_data[find_max_x,0] + object_data[find_min_x,0]) / 2
            box_y = (object_data[find_max_x,1] + object_data[find_min_x,1]) / 2
            # box_w = 
...
for i in range(len(file_list)):
    ...
    if len(cur_object) != 0:
        ...
        for box in cur_box_dict.values():
            points, lines = draw_3d_bbox(box[0], box[1], box[2], box[3], box[4], box[5])
