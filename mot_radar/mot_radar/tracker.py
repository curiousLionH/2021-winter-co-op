...
class Tracks(object):
    ...
    def __init__(self, detection, trackId,v_x,v_y,object_points,dt,index, box_dict = None):
    ...
    def update(self, detections,v_x, v_y, cluster_data,dt, box_dict = None):
        ...
        if len(self.tracks) == 0:
            ...
            # for key in box_dict.keys():
                ...
                # print(detections[i])
                track = Tracks(detections[i], self.trackId,v_x, v_y, object_points[:,:3],dt, detections[i][6])
                # print(f"track : {track}")
                self.trackId += 1
        ...
        if(len(un_assigned_detects) != 0):
            for i in range(len(un_assigned_detects)):
                ...
                track = Tracks(detections[un_assigned_detects[i]], self.trackId,v_x,v_y,object_points_2[:,:3],dt, detections[i][6])
                self.trackId +=1
                self.tracks.append(track)
