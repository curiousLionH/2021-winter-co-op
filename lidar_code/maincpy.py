...
def make_object_list(cluster_data):
    ...
    for i in range(max_label+1):
        ...
        if len(object_data) != 0:
            ...
            object_list = np.reshape(object_list,(-1,8))        # (num개수, 방금 append한 12개 특징) 의 array로 reshape
            point_list = np.append(point_list,[i,rect[0], rect[1], rect[2], rect[3], find_min_z, find_max_z])
            point_list = np.reshape(point_list,(-1,7))

    return object_list, point_list
...
for i in range(len(file_list)):
    ...
    cur_object_list, cur_point_list = make_object_list(xyz_labels)
    ...
    cur_point_list = cur_point_list[rate_exclusion]
    ...
    cur_point_list = cur_point_list[volume_exclusion]

    tracking_object = np.array([])
    tracking_object = cur_object_list[0:9]
    

    tracker.update(tracking_object, 0 , 0 , xyz_labels, 0.1) #### 임의 
    ...
    if len(cur_object) != 0:
        ...
        k = 0
        while k == 0:
        #for i in range(int(len(cur_point_list[i,:])/5)):
            #purpose = np.where(cur_point_list[:,0] == i , True, False)
            #cur_point_list = cur_point_list[purpose]
            df = pd.DataFrame(cur_point_list)
            points, lines = draw_3d_bbox(cur_point_list[i,1],cur_point_list[i,3],cur_point_list[i,2],cur_point_list[i,4],cur_point_list[i,5],cur_point_list[i,6])
