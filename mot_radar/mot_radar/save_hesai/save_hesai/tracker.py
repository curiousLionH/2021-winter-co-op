import numpy as np 
from save_hesai.kalmanFilter import KalmanFilter
...
class Tracks(object):
    ...
    def __init__(self, detection, trackId,v_x,v_y,object_points,dt):
        ...
        self.KF = KalmanFilter(detection.reshape(7,1),v_x,v_y,dt)
        self.KF.predict(dt)
        self.KF.correct(detection.reshape(7,1),1,dt)
        self.object_points = object_points
        # self.KF.correct(np.matrix(detection).reshape(2,1))
        self.trace = deque(maxlen=10)
        self.prediction = detection.reshape(1,7)
...
class Tracker(object):
    ...
    def update(self, detections,v_x, v_y, cluster_data,dt):
        ...
        for i in range(len(assignment)):
            if assignment[i] != -1:
                # print('cost,ass',cost[i][assignment[i]])
                if (cost[i][assignment[i]] > self.dist_threshold):
                    assignment[i] = -1
                    un_assigned_tracks.append(i)
            else:
                self.tracks[i].skipped_frames +=1
                # print('frame',self.tracks[0].skipped_frames)
        ...
        for j in range(len(assignment)):
            if(assignment[j] != -1):
                ...
                self.tracks[j].prediction = self.tracks[j].KF.correct(np.array([[0],[0],[0],[0],[0],[0]]),0,dt)
